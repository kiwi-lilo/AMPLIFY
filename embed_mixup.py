...
from mixup_model.fwdbertmodel import MyBertModel
...
class pretraining_model(nn.Module):
    def __init__(self,
        ...
        if 'roberta_base' in self.model_name_or_path:
            ...
            self.model = MyBertModel.from_pretrained(f"{self.model_name_or_path}", num_labels=num_labels)
    ...
    def forward(self, inputs,labels=None):
        ...
        lam = max(lam,1-lam)
        outputs = self.model.forward_mix_embed(
            x1 = input_ids, 
            att1=attention_mask, 
            x2 = input_ids_perm, 
            att2=attention_mask_perm, 
            lam = lam
        )
        pooled_output = outputs[1]
        pooled_output = self.dropout(pooled_output)
