...
from mixup_model.tmixBertModel import BertModel
...
class pretraining_model(nn.Module):
    ...
    def get_perm(self, x):
        ...
        batch_size = x.size()[0]
        return torch.randperm(batch_size).cuda()

    def compute_loss(self, logits, labels,labels_perm, lam):
        criterion = nn.CrossEntropyLoss()
        return lam * criterion(logits, labels) + (1 - lam) * criterion(logits, labels_perm)

    def forward(self, inputs,labels=None):
        input_ids = inputs['input_ids']
        attention_mask = inputs['attention_mask']
        # token_type_ids=inputs["token_type_ids"]
        index = self.get_perm(input_ids)
        # input_ids_perm = input_ids.clone()[index]
        # attention_mask_perm = attention_mask.clone()[index]
        # token_type_ids_perm = token_type_ids.clone()[index]
        mixlayer = np.random.choice([7, 9, 12])
        mixlayer = mixlayer - 1
        ...
        if labels is not None:
            labels_perm = labels[self.get_perm(labels)]
            loss = self.compute_loss(logits, labels, labels_perm, lam)
        return logits,loss
    def forward_pre(self, inputs,labels=None):
        input_ids = inputs['input_ids']
        attention_mask = inputs['attention_mask']
        index = self.get_perm(input_ids)
        index = index[torch.argsort(index)]
        mixlayer = np.random.choice([7, 9, 12])
        mixlayer = mixlayer - 1
        lam = np.random.beta(0.2, 0.2)
        lam = max(lam,1-lam)
...
class TrainingModule(LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        labels = batch['labels']
        inputs = {k: v for k, v in batch.items() if k != 'labels'}
        logits,loss = self.model.forward_pre(inputs,labels)
        return {"loss": loss, "preds": logits, "labels": labels}

    def test_step(self, batch, batch_idx, dataloader_idx=0):
        inputs = {k: v for k, v in batch.items() if k != 'labels'}
        logits,loss = self.model.forward_pre(inputs)
    ...
    def validation_epoch_end(self, outputs):
        ...
        # val_acc = self.model.lam*accuracy_score(valid_true, valid_pred) + (1-self.model.lam)*accuracy_score(labels_perm, valid_pred)
        val_acc = accuracy_score(valid_true, valid_pred)
        val_f1 = f1_score(valid_true, valid_pred, average='macro')
        # val_f1 = self.model.lam*f1_score(valid_true, valid_pred, average='macro') + (1-self.model.lam)*f1_score(labels_perm, valid_pred, average='macro')
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        checkpoint_callback = ModelCheckpoint(
            ...
            filename= "{Config.basic.task_name}_{epoch}_{val_acc:.6f}_{val_f1:.6f}_",
