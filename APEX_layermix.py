...
from mixup_model.layermix import BertModel
...
class pretraining_model(nn.Module):
    def __init__(self,
        ...
        if 'roberta_base' in self.model_name_or_path:
            ...
            self.model = BertModel.from_pretrained(f"{self.model_name_or_path}", num_labels=num_labels)
    ...
    #     )
    #     return outputs
    def loss(self, logits, labels,labels_copy,alpha):
        ...
        loss = alpha*loss_fnc(logits, labels) + (1-alpha)*loss_fnc(logits, labels_copy)
        return loss

    def forward(self, inputs, labels=None):
        self.perindex = torch.randperm(inputs["input_ids"].shape[0]).cuda()
        self.alpha = max(torch.distributions.beta.Beta(0.1, 0.1, validate_args=None).sample((5,)).cuda())
        mixlayer = 11
        input_ids = inputs["input_ids"]
        token_type_ids=inputs["token_type_ids"]
        attention_mask=inputs["attention_mask"]
        ...
        _loss = 0
        labels_copy = labels[self.perindex]
        if labels is not None:
            _loss = self.loss(features,labels,labels_copy,self.alpha)
        return features, _loss,labels_copy
        
    def forward_nor(self, inputs, labels=None):
        self.perindex = torch.randperm(inputs["input_ids"].shape[0]).cuda()
        self.perindex = self.perindex[torch.argsort(self.perindex)]
        self.alpha = max(torch.distributions.beta.Beta(0.1, 0.1, validate_args=None).sample((5,)).cuda())
        mixlayer = 11
        input_ids = inputs["input_ids"]
        token_type_ids=inputs["token_type_ids"]
...
class TrainingModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            labels = batch['labels']
            inputs = {k: v for k, v in batch.items() if k != 'labels'}
            logits, loss,logits_copy = self.model(inputs, labels)
        except:
            print("error")
        return loss
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        labels = batch['labels']
        inputs = {k: v for k, v in batch.items() if k != 'labels'}
        logits, val_loss,logits_copy = self.model.forward_nor(inputs, labels)
        return {"loss": val_loss, "preds": logits, "labels": labels,"preds_copy": logits_copy}
    ...
    def validation_epoch_end(self, outputs):
        ...
        preds_copy = torch.cat([x["preds_copy"] for x in outputs])
        # print('alpha',outputs["alpha"])
        # alpha = outputs["alpha"]
        loss = torch.stack([x["loss"] for x in outputs]).mean()

        pred = [item.argmax(axis=-1) for item in preds.detach().cpu().numpy()]
        label = [item for item in labels.detach().cpu().numpy()]
        preds_copy = [item for item in preds_copy.detach().cpu().numpy()]
        # val_acc = self.model.alpha*accuracy_score(label, pred) + (1-self.model.alpha)*accuracy_score(preds_copy, pred)
        val_f1 = f1_score(label, pred, average='macro')
        val_acc = accuracy_score(label, pred)
        # val_f1 = self.model.alpha*f1_score(label, pred, average='macro') + (1-self.model.alpha)*f1_score(preds_copy, pred, average='macro')
        self.log("val_loss", loss, prog_bar=True)
        self.log("val_acc", val_acc, prog_bar=True)
        self.log("val_f1", val_f1, prog_bar=True)
        return loss
